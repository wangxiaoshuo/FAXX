/**
 * Created by karl on 2017/1/20.
 */
var down = false;

function toggleDown(parameter) {
    if(parameter) {
        if (down == false) {
            down = true;
            t1 = new Tween(document.getElementById('menu_holder').style, 'top', Tween.strongEaseOut, -66, 0, .6, 'px');
            t1.start();
            // $('#hit_area').css({"height": "78px"});
        }
    }
}
function toggleUp() {
    if (down == true) {
        down = false;
        t1 = new Tween(document.getElementById('menu_holder').style, 'top', Tween.strongEaseIn, 0, -66, .4, 'px');
        t1.start();
        // $('#hit_area').css({"height": "12px"});
    }
}
function Delegate() {}
